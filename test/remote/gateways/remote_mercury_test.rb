...
class RemoteMercuryTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @credit_card = credit_card('4895281000000006', brand: 'visa', month: '12', year: '25')
  ...
  def test_failed_authorize
    response = @gateway.authorize(257, @credit_card, @options)
  ...
  def test_failed_purchase
    response = @gateway.purchase(257, @credit_card, @options)
    assert_failure response
    assert_equal 'DECLINE', response.message
